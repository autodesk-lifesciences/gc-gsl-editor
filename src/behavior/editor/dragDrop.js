  ...
  switch(payload.item.id) {
    default:
      ...
      switch(payload.item.position) {
        case 'prefix':
          var insertPosition = { row: dragPosition.row, column: token.start }; 
          ace.editor.env.document.insert(insertPosition, payload.item.text);
          var cursorPosition = { row: insertPosition.row, column: insertPosition.column + 1};
          ace.editor.moveCursorToPosition(cursorPosition);
          break;     
        case 'postfix':
          var insertPosition = { row: dragPosition.row, column: token.start + token.value.length};
          ace.editor.env.document.insert(insertPosition, payload.item.text);
        ...
        default:
          ...
          var cursorPosition = { row: dragPosition.row, column: dragPosition.column+ payload.item.text.length};
          ace.editor.moveCursorToPosition(cursorPosition);
          break;      
      }
      break;
